...
async def handle_web_app_data(message: types.Message):
    ...
    if data.get('action') == 'exchange':
        ...
        # Создаём инвойс на 1 звезду
        await bot.send_invoice(
            chat_id=message.chat.id,
            title="🐟 Обмен монет на TON Fish",
            description=f"Обмен {coins} монет → {coins} TON Fish на кошелёк {wallet[:12]}...",
            payload=json.dumps({"coins": coins, "wallet": wallet, "user_id": user.id, "username": user.username or ""}),
            currency="XTR",
...
async def successful_payment(message: types.Message):
    """Оплата прошла — уведомляем игрока и тебя"""
    try:
        ...
        await message.answer("❌ Ошибка обработки. Напиши администратору.")
        return
