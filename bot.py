...
# специфика передачи прав), старый ID (-5478312122) перестал существовать как чат
# для отправки вообще, и bot.send_message на него падал с TelegramMigrateToChat, а
# не просто "нет прав". /checkgroup поймал это и вытащил настоящий новый chat_id
# прямо из ответа Telegram (поле migrate_to_chat_id) — он и зашит здесь теперь.
# _support_group_chat_id — актуальный chat_id группы В ПАМЯТИ ЭТОГО ПРОЦЕССА, не
# обязательно равен константе выше. Если группа мигрирует ЕЩЁ раз в будущем (бывает
# при смене настроек супергруппы), notify_support_group() сам на лету переключится
# на новый ID и пересошлёт застрявшее сообщение — но это лечение живёт только до
# следующего перезапуска бота, так что новый ID всё равно придётся аналогично
# перенести в эту константу и задеплоить (см. алерт в личку при такой миграции).
...
async def starttournament_command(message: types.Message):
    ...
    try:
        ...
        text = (
            ...
            "Призовой фонд — 500 Stars ⭐ на троих:\n"
            "🥇 1 место — 250⭐\n"
            "🥈 2 место — 150⭐\n"
            "🥉 3 место — 100⭐\n\n"
            ...
            "Prize pool — 500 Stars ⭐ for the top 3:\n"
            "🥇 1st place — 250⭐\n"
            "🥈 2nd place — 150⭐\n"
            "🥉 3rd place — 100⭐\n\n"
